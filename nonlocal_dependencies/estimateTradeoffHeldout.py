...
def calculateMemorySurprisalTradeoff(train, dev, args): #Q: what is train and dev? where are these lists taken from?
    """ Input: 
        ...
        - args (dict) with entries
           ...
           - gamma (int, e.g. 1): threshold for choosing between interpolation and backoff. If gamma > 1, rare prefixes are not used for interpolation, and trigger backoff instead.
                    ##  In backoff, we use the trigram if the evidence is
        ...
        Returns:
          - auc: Area below the memory-surprisal tradeoff curve
                ## i.e. the measure of IL
          - devSurprisalTable: lists surprisal by n, from n=0 up to cutoff
    """

    itos = list(set(train) | set(dev))
    # For the algorithm, we will need to reverse the corpora (list)
    dev = dev[::-1] #[::-1] is just a python shortcut to reverse the list
    train = train[::-1]

    # idev and itrain are suffix arrays: They contain indices to positions in the text,
    # ordered by the alphabetical position of the suffixes starting at them.
    idev = range(len(dev))
    itrain = range(len(train))


    # what is the range dev[1:21), e.g.?? 20 words?? wdym sort by that
    idev = sorted(idev, key=lambda i:dev[i:i+20]) #to determine sort order, feed each value in idev into lambda fn -> sort by range(i, i+20) in dev
    itrain = sorted(itrain, key=lambda i:train[i:i+20])

    # for i in range(50000, 140000):
    #     print(dev[idev[i]])
    ...
    # Helper function. For a given span length k (>= 0), computes for each position in the heldout set (`dev`),
    # the location of the first and last entries in itrain
    # where the same string of length k starts as after the given position in idev.
        # for matching words in dev set to training set for future frequency calcs??
    def getStartEnd(k): # k is for range(0, args.cutoff), so corresponds to N where N is context taken into account
    ...
    for k in range(0,args.cutoff):
       ...
       for j in range(len(idev)):
          # print(dev[idev[j]])
          # if item is either "PAD" for padding or "SOS" for start-of-sentence symbol, skip
